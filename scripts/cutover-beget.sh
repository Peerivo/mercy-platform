...
rollback_target() {
  [[ -n "${backup_prefix}" ]] || {
    echo "::error::Target rollback requested before safety backup path was recorded."
    return 1
  }

  ssh "${REMOTE}" bash -s -- "${SUPABASE_DIR}" "${backup_prefix}-postgres.dump" <<'REMOTE'
set -euo pipefail
supabase_dir="$1"
backup_file="$2"
cd "$supabase_dir"

mapfile -t services < <(docker compose config --services | grep -v '^db
  rc=$?
  set +e
  if [[ "${success}" != "1" && "${target_mutated}" == "1" ]]; then
    echo "Migration did not complete after target mutation; restoring the Beget safety backup."
    rollback_target || echo "::error::Automatic Beget rollback failed; retained backup: ${backup_prefix}-postgres.dump"
  fi
...
ssh "${REMOTE}" "docker exec -i supabase-db pg_restore -l < '${backup_prefix}-postgres.dump' >/dev/null"
...
[[ "$state" == "0||0|0" ]] || {
  echo "Target rollback verification failed: $state" >&2
  exit 1
}
