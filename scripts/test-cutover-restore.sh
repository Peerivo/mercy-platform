...
# --use-list excludes database privileges encoded in archive --create
# metadata. Apply the archived database GRANT/REVOKE statements only.
archive_acl_sql="$(docker run --rm -i postgres:17-alpine pg_restore --create --schema-only -f - < "$dump_file" |
  awk '/^(REVOKE|GRANT) .* ON DATABASE mercy_restore_test (FROM|TO) .*;$/ {print}')"
[[ "$(printf '%s\n' "$archive_acl_sql" | awk '/^REVOKE / {r++} /^GRANT / {g++} END {print r+0 "|" g+0}')" == "1|3" ]]
printf '%s\n' "$archive_acl_sql" |
  docker exec -i "$db_container" psql -U supabase_admin -d template1 -v ON_ERROR_STOP=1 >/dev/null
unset archive_acl_sql
