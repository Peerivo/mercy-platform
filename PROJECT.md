...
- Migration preflight для перехода Mercy на Beget запускается вручную через GitHub Actions и fail-closed проверяет production secrets, pinned SSH host key, source/target DB versions, пустой target Mercy contour и требуемые Postgres extensions. Source DB password не передаётся в process argv; preflight не выгружает пользовательские данные и не изменяет Beget.
