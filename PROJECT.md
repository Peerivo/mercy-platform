...
- Cutover workflow `Cut over Mercy Supabase to Beget` требует явный выбор `MIGRATE`, повторяет compatibility gates, запрещает Storage/MFA/SSO/non-email migration profile, делает backup цели, freeze source атомарной транзакцией с Docker stdin `-i` и проверкой покрытия по конкретным relations, переносит Auth/public data, автоматически unfreeze-ит source при failure и оставляет source frozen только после полной target verification.
