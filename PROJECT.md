...
## IMPLEMENTED
Публичная главная, каталог/гео-RPC, Auth flows, приватная форма обращения, кабинет, история/Realtime чат с привязанной к обращению RPC-idempotency и атомарной пользовательской квотой, просмотр плана, предложение помощи, быстрый выход с BFCache guard; строгая схема, RLS/grants, assignment/status/staff-role RPC, audit, consent and IdentityLink contract; CI, controlled migration deployment, docs and Docker. Disposable Supabase harness воспроизводит схему и запускает pgTAP, настоящие Auth/API/Realtime (включая открытые соединения после переназначения и отзыва роли) и минимальные Chromium-сценарии без облачных credentials.

## IN_PROGRESS
Полный браузерный UI рабочего места администратора/координатора остаётся в работе. Исправленный integration suite должен быть фактически подтверждён GitHub-hosted runner с Docker; выполнение конкретного SHA фиксируется отдельно и не подменяется успешным применением миграций.
