...
- Cutover workflow `Cut over Mercy Supabase to Beget` требует явный выбор `MIGRATE`, повторяет compatibility gates, запрещает Storage/MFA/SSO/non-email migration profile, делает backup цели, freeze source атомарной транзакцией с Docker stdin `-i` и проверкой покрытия по конкретным relations, применяет schema + Auth/public data + migration history одной target-транзакцией через remote shell, изолированный от локальной подстановки переменных, нормализует исторический specialist Storage bucket к подтверждённому пустому source-профилю и автоматически unfreeze-ит source при failure и оставляет source frozen только после полной target verification.
