...
- Chromium: 2 production-build browser scenarios configured, including two contexts at 360 px, Quick Exit, ordinary History return and an assertion that Chromium really emitted `pageshow` with `persisted=true`; the Chromium project explicitly omits Playwright's default `--disable-back-forward-cache` launch argument so that this exercises a real BFCache restoration rather than making it impossible in the test runner. Assertions cover removal of both private chat text and a restored form value, `/safe` loop avoidance, and explicit return.
