...
- REG.RU application deployment must require explicit `DEPLOY`, current live `main` SHA, pre-verified `REG_RU_KNOWN_HOSTS`, an immutable image, candidate environment that is not promoted before local data + public TLS smoke, rollback on error/HUP/INT/TERM, exact published SHA verification, and retention of only the current plus one rollback image. `/health/data` must return no user data and must fail when the configured public Supabase RPC path is unavailable. The app deployment workflow must never apply database migrations.
