...
- Chromium: 3 production-build browser scenarios configured. The two-user 360 px scenario independently covers registration/session isolation, request/chat/refresh, foreign-request denial, Quick Exit, ordinary Back, removal of private chat and draft text, explicit return and revoked local session. A separate BFCache scenario uses document-level `page.goto`/Back transitions and requires `pageshow.persisted=true` on the same protected document before the early guard returns it to `/safe`; observations remain in the Playwright process rather than the page storage that Quick Exit clears. Safe `pageshow`/`pagehide` diagnostics contain only phase, `persisted`, a route template and random document ID, while CDP captures `Page.backForwardCacheNotUsed` reasons on failure. The Chromium project omits Playwright's default `--disable-back-forward-cache` argument and the scenario also verifies the actual browser command line. The callback rejection remains independent.
