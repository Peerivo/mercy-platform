...
Run #10 reported `missing_object` at TOC `4300` twice while preserving the retained backup and leaving the source unfrozen. The protected read-only `Inspect retained Beget safety archive` workflow inspected failed cutover run `35770879007` and TOC `4300`. It verifies the run identity, reads one `pg_restore -l` index line without SQL/parameter values, and does not drop or restore a database. Run `35832828340` identified the missing GraphQL wrapper; retry only after the reviewed fix and disposable integration pass. At that point the Beget target was unverified.
...
**Security gate:** в логах старого recovery run #8 оказалось значение настройки JWT. До отдельного устранения инцидента с логами/credential не считать безопасным свежий approval на `MIGRATE`; само выполнение `MIGRATE` требует нового явного разрешения. DNS и приложение REG.RU в рамках recovery не изменялись.
