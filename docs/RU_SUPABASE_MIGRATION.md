...
4. Take and retain a safety backup of the fresh Beget target using a custom-format `pg_dump --create`, globals backup, and a server-side fingerprint of database owner/ACL/settings; the archive must contain database creation metadata and is considered restorable only after the server writes its `prepared` state marker and metadata fingerprint.
...
If a cutover fails or is interrupted after the Beget schema/data transaction has committed, the source is unfrozen and the target must not be reused implicitly. New runs normally restore their own current safety backup automatically on such failures. For a previously failed/interrupted cutover, rerun the guarded workflow with `MIGRATE` and set `recover_from_run_id` to the exact GitHub Actions run ID. The workflow first queries the GitHub Actions API and requires that ID to belong to the main-branch `Cut over Mercy Supabase to Beget` workflow with terminal conclusion `failure`, `timed_out`, or `cancelled`; a successful, nonterminal, or mistyped run ID is rejected before any Beget restore. The script restores only `beget-pre-migration-backups/before-<run-id>-postgres.dump`, refuses explicit recovery if a durable successful-cutover marker exists, stops non-DB containers belonging to the same Docker Compose project using full, untruncated Docker container IDs so `supabase-db` is excluded reliably, recreates the `postgres` database so post-backup Mercy objects/extensions cannot survive, restores database-level metadata plus archived object ownership/ACLs, verifies the database metadata fingerprint and Auth/Storage service-role access and the fresh target profile (`0 users`, no `public.help_requests`, `0 buckets`, `0 objects`), then takes a new safety backup and proceeds with the normal cutover. New-format backups are eligible for destructive restore only when the archive contains database creation metadata and the matching `prepared` marker plus metadata fingerprint are present. The sole legacy compatibility exception is retained run `35770879007`, which predates `--create`/prepared markers and uses a one-time database-metadata donor captured before the database is dropped. Any other legacy/partial, missing, empty, nonnumeric, or non-fresh recovery backup fails closed before source freeze.

The CI disposable Supabase job also exercises the recovery strategy against a real PostgreSQL database: it takes a custom-format baseline dump with non-default owner/ACL and database setting, revokes `PUBLIC CONNECT`, creates a post-backup object, recreates the database, restores the dump, and verifies that the extra object is gone while ownership, grants, settings, and the `PUBLIC CONNECT` denial survive.

### Recovery-only workflow

Select `RECOVER` and the exact failed/interrupted run ID to restore the retained target safety backup. This mode finishes after the fresh-target and service-role checks. It neither freezes Mercy-prod nor applies migrations or starts `MIGRATE`. Confirm source remains unfrozen and rerun the read-only preflight, including users, Mercy relation marker, Storage buckets, and Storage objects. A later cutover requires a separate fresh `MIGRATE` approval. Keep retained backups until a complete successful cutover.
