...
7. Freeze all `public` table writes and Auth user/identity writes on the old source using temporary DB triggers; the runner streams the SQL to containerized `psql` with Docker stdin kept open (`-i`), then verifies the exact expected relation set has the trigger and reports any missing relation. The failure trap removes these triggers automatically.
