...
After failed recovery runs #8 and #9, the Beget database has been dropped and partially recreated; do not assume it is fresh or serving correctly. The source was never frozen by `RECOVER`. Retained run #7 safety files remain. Retry only with reviewed code using the local `supabase_admin` superuser, verified before database drop, because the archived database settings include restricted custom GUCs. `pg_restore` SQL diagnostics must be suppressed in Actions logs: they can disclose confidential database settings. Run #8 emitted one such value and requires separate credential/log remediation. Run #9 suppressed raw SQL but needs a safe classified diagnostic to locate its restore failure without logging values. For the legacy run #7 archive, compare restored database settings by presence and successful complete archive replay rather than hashing the partially restored run #8 target as a baseline; new backups retain strict saved fingerprint comparison. Confirm a successful recovery and fresh read-only preflight before any later MIGRATE approval.
...
The first read-only inspection, run `35829539729`, found `4300; 0 0 ACL graphql_public FUNCTION graphql("operationName" text, query text, variables jsonb, extensions jsonb) supabase_admin`. This is archive index metadata; it does not show whether the function, schema, extension, or a grant dependency is missing. Use the same protected inspector in `GRAPHQL_CONTEXT` mode to print bounded related TOC metadata and boolean target `pg_catalog` flags. Do not attempt another destructive recovery until a reviewed fix explains the failure and passes disposable integration. Neither diagnostic mode prints SQL or database setting values.

Временный read-only запуск инспектора после protected merge в `main` обусловлен недоступностью UI `workflow_dispatch`; его `push` trigger ограничен изменением файла workflow, failed run `35770879007` и `GRAPHQL_CONTEXT`. Он только читает индекс архива и флаги каталога, не делает `RECOVER`. После диагностики удалить временный триггер через проверенный PR.

Read-only run `35832828340` показал: архив содержит `SCHEMA graphql_public`, `EXTENSION pg_graphql`, ACL `graphql_public.graphql(...)`, но не содержит `FUNCTION` definition; на Beget флаги `t|f|t|t` (схема есть, wrapper отсутствует, расширение установлено и доступно). Это объясняет `missing_object toc=4300`: ACL применяется к отсутствующей функции. Исправленный recovery переносит только этот ACL в конец replay, восстанавливает весь остальной архив с owners/grants, при отсутствии функции создаёт официальный Supabase wrapper через `graphql.resolve`, затем применяет сохранённый ACL и проверяет owner, API-role grants и вызов от `anon`. CI воспроизводит extension-member архив, в котором pg_dump оставляет ACL без определения функции. Временный `push` trigger удалён; до успешного `RECOVER` и read-only preflight Beget остаётся непроверенным.
