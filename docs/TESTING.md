...
- настоящие `postgres_changes` подписки без REST polling/моков: подтверждённая доставка разрешённым клиентам и отсутствие доставки U2/C1 после переназначения при сохранённом сокете; повторное подключение и отзыв staff-role;
...
Готовность Realtime означает не только Phoenix-статус `SUBSCRIBED`, но и отдельное channel-scoped `system`-подтверждение с `extension=postgres_changes`, `status=ok`; обработчики ставятся до `subscribe`, оба сигнала принимаются в любом порядке. Это используется при первом соединении и восстановлении, после чего чат догружает доступную историю и сливает её по UUID. Ошибка system, `CHANNEL_ERROR`, `TIMED_OUT`, неожиданное закрытие или 15-секундный timeout завершают ожидание ошибкой.

Отрицательная проверка отличается от ожидания доставки: обработчики фиксируют запрещённые события с момента отправки уникального контрольного сообщения, а после подтверждённой доставки разрешённым участникам соединения остаются открыты ещё на ограниченное двухсекундное окно наблюдения. Запоздалое событие или потеря проверяемого соединения проваливает сценарий. Это окно применяется к C1/C2, U2 и переподключениям; оно не используется для маскировки готовности или повторных пробных отправок. Publication содержит только INSERT/UPDATE/DELETE протокол таблицы `messages`, но таблица не использует FULL replica identity: приватное тело не попадает в старый DELETE row. Публичные Broadcast-каналы не используются.
...
## Результат текущей локальной проверки (2026-09-13)

- **VERIFIED:** lint завершён без ошибок (два существующих style warning), strict typecheck, 7 unit-тестов и production build.
