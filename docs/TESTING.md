...
Integration harness создаёт только вымышленные `U1`, `U2`, `C1`, `C2`, `A1` с адресами домена `.invalid`. Service-role локального проекта используется исключительно для создания Auth fixtures и исходных ролей. Все утверждения о доступе выполняются клиентами с JWT соответствующего пользователя; приложение service-role key не получает.
...
- допустимые/недопустимые переходы, неизменяемость сообщений, nonce-idempotency и уникальное конкурентное назначение;
