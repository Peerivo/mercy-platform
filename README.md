...
MVP добровольной помощи и приватной координации. Требования: Node 22+, npm, внешний Supabase; Docker/Supabase CLI для integration.
...
## Тестовые сценарии
Job `supabase-integration` сам поднимает полный disposable Supabase (PostgreSQL/PostGIS, Auth, PostgREST, Realtime и Inbucket), применяет миграции с нуля и только затем создаёт USER, COORDINATOR и ADMIN через Auth API. Облачный проект и repository secrets не нужны. Вымышленные организации имеют название «Тестовая…», аккаунты — домен `.invalid`.
