...
describe.sequential("disposable Supabase security boundary", () => {
  beforeAll(async () => {
    ...
    for (const name of ["u1", "u2", "c1", "c2", "a1"]) {
    ...
    await Promise.all(["u1", "u2", "c1", "c2", "a1"].map(signIn));
  }, 30_000);
  ...
  test("anonymous sees only doubly verified, non-confidential catalog rows", async () => {
    ...
    await svc.from("service_locations").insert([
  ...
  test("database message rate limit rejects excess writes", async () => {
    let rejected = 0;
    for (let i = 0; i < 22; i++) {
      const result = await clients.u1.rpc("send_message", { case_id: case1, message_body: `rate check ${i}`, message_nonce: crypto.randomUUID() });
      if (result.error) rejected++;
    }
    expect(rejected).toBeGreaterThan(0);
    const svc = createClient(url, serviceKey);
    const since = new Date(Date.now() - 60_000).toISOString();
    const count = await svc.from("messages").select("id", { count: "exact", head: true }).eq("author_id", ids.u1).gte("created_at", since);
