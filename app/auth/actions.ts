...
export async function signIn(fd: FormData) {
  const x = creds.parse(Object.fromEntries(fd));
  const s = await serverSupabase();

  const { error } = await s.auth.signInWithPassword(x);

  if (error) {
    console.error("AUTH SIGNIN:", error.code, error.message);
    redirect(`/auth?error=${encodeURIComponent(error.code ?? "signin")}`);
  }
...
export async function signUp(fd: FormData) {
  ...
  if (error) {
    ...
    redirect(`/auth?error=${encodeURIComponent(error.code ?? "signup")}`);
  }
