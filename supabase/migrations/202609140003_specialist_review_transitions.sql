-- Correct specialist review transitions and require coverage for every normalized
-- medical service category. Review and audit stay in one locked transaction.
...
begin
  ...
  if new_qualification = 'VERIFIED' and exists (
    select 1
    from unnest(profile.services) as service(category)
  ...
  insert into public.specialist_status_events(
    ...
    specialist, auth.uid(), new_publication, new_qualification, trim(reason_text)
  );
end
$$;
